...
def explain_prediction(drug1_fp, drug2_fp, model, le):
    """
    Given two Morgan fingerprints, a trained XGBoost model, and its
    LabelEncoder, return the predicted interaction label and the
    top 10 fingerprint bits driving that prediction.
    """
    ...
    contribs = model.get_booster().predict(dmat, pred_contribs=True)  # (1, 54, 4097)
